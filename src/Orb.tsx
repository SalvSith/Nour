...
const frag = /* glsl */ `
  ...
  vec3 mixOklab(vec3 a, vec3 b, float t) {
    vec3 aLab = linear_srgb_to_oklab(a);
    vec3 bLab = linear_srgb_to_oklab(b);
    return max(oklab_to_linear_srgb(mix(aLab, bLab, t)), 0.0);
  }
  ...
  float gameBlobSDF(vec2 uv, vec4 gb) {
    // Un-scale Y so blobs measure distance in screen-proportional space,
    // not Y-compressed UV space — this makes them appear circular, not squished.
    float yInv = 1.0 / (1.0 + gameMode * 1.1);
    vec2 rel = vec2(uv.x - gb.x, (uv.y - gb.y) * yInv);
    float dist = length(rel);
    // Two noise octaves keyed to blob position → every blob has a unique,
    // slowly morphing silhouette, matching the organic satellite blobs.
    // (Third high-freq octave dropped — amplitude 0.004 is imperceptible but costly on mobile.)
    float n = snoise3(vec3(rel * 2.8, iTime * 0.12 + gb.x * 4.9 + gb.y * 3.1)) * 0.018
            + snoise3(vec3(rel * 5.8, iTime * 0.25 + gb.x * 7.7 - gb.y * 5.3)) * 0.009;
    return dist - gb.z * gb.w - n * gb.w;
  }

  vec4 draw(vec2 uv) {
    // Rainbow spin value -- always running, used for the euphoric colour path only
    float rainbowSpin = iTime * 80.0;

    // Valence tint for the normal colour path
    ...
    vec3 c1_norm = applySaturation(adjustHue(baseColor1, normalHue + drift1 + audioBass * 130.0) * baseTint, liveSat);
    vec3 c2_norm = applySaturation(adjustHue(baseColor2, normalHue + drift2 - audioTreble * 100.0) * baseTint, liveSat);
    vec3 c3_norm = applySaturation(adjustHue(baseColor3, normalHue + drift3 + audioMid * 70.0) * baseTint, liveSat);

    // Euphoric colour path -- three triadic hues all spinning, max saturation, no tint
    vec3 c1_rave = applySaturation(adjustHue(baseColor1, rainbowSpin + audioBass * 130.0), 1.3);
    vec3 c2_rave = applySaturation(adjustHue(baseColor2, rainbowSpin + 120.0 - audioTreble * 100.0), 1.3);
    vec3 c3_rave = applySaturation(adjustHue(baseColor3, rainbowSpin + 240.0 + audioMid * 70.0), 1.3);

    // Crossfade between paths entirely in colour space -- euphoric never touches hue math,
    // so the transition is a smooth RGB blend with no sudden hue jumps
    vec3 color1 = mix(c1_norm, c1_rave, euphoric);
    vec3 color2 = mix(c2_norm, c2_rave, euphoric);
    vec3 color3 = mix(c3_norm, c3_rave, euphoric);

    if (trickActive > 0.01) {
      float tLen = length(uv);
      float tAng = atan(uv.y, uv.x);
      vec3 trickCol;

      if (trickType < 0.5) {
        // "Bloom" — Sunset Coral → Petal Rose. Pure smooth radial.
        vec3 tA = vec3(1.0, 0.29, 0.14);
        vec3 tB = vec3(1.0, 0.41, 0.76);
        float g = clamp(tLen * 1.5 + sin(iTime * 0.7) * 0.14, 0.0, 1.0);
        trickCol = mix(tA, tB, g);

      } else if (trickType < 1.5) {
        // "Pendulum" — Ice White → Deep Sapphire. Smooth lateral sweep.
        vec3 tA = vec3(0.72, 0.93, 1.0);
        vec3 tB = vec3(0.04, 0.20, 0.82);
        float g = clamp(uv.x * 1.0 + 0.5, 0.0, 1.0);
        trickCol = mix(tA, tB, g);

      } else if (trickType < 2.5) {
        // "Shatter" — Neon Lime → Electric Yellow. Smooth radial + gentle angular wave.
        vec3 tA = vec3(0.16, 1.0, 0.14);
        vec3 tB = vec3(0.98, 1.0, 0.0);
        float g = clamp(tLen * 1.3 + sin(tAng * 1.5 + iTime * 0.4) * 0.22, 0.0, 1.0);
        trickCol = mix(tA, tB, g);

      } else if (trickType < 3.5) {
        // "Drop" — Deep Crimson → Molten Gold. Smooth vertical — hot pools downward.
        vec3 tA = vec3(0.90, 0.04, 0.16);
        vec3 tB = vec3(1.0, 0.62, 0.0);
        float g = clamp(-uv.y * 1.1 + 0.5, 0.0, 1.0);
        trickCol = mix(tA, tB, g);

      } else {
        // "Pulses" — Abyss Navy → Luminous Aqua. Low-frequency smooth radial pulse.
        vec3 tA = vec3(0.02, 0.10, 0.56);
        vec3 tB = vec3(0.28, 1.0, 0.87);
        float g = clamp(sin(tLen * 3.2 - iTime * 2.8) * 0.5 + 0.5, 0.0, 1.0);
        trickCol = mix(tA, tB, g);
      }
    ...
    float effLen = mix(len, length(vec2(uv.x / lavaX, uv.y)), gameMode);

    float emotionRadius = mix(0.05, 0.8, mix(emotionSize, 0.86, euphoric));
    float happyV = max(emotionValence, 0.0);
    float dynamicRadius = emotionRadius + breathe * (0.03 + happyV * 0.015) * (0.5 + emotionArousal * 1.5) + audioLevel * 0.04;
    dynamicRadius *= 1.0 + gameMode * 1.6;
    float audioPow = audioLevel * audioLevel;

    float arousalScale = 0.3 + emotionArousal * 1.4;

    float gameNoiseMul = 1.0 + gameMode * 2.0;
    float bassD = (audioBass * 0.16 + audioBass * audioBass * 0.11) * gameNoiseMul;
    float midD = (audioMid * 0.10 + audioMid * audioMid * 0.07) * gameNoiseMul;
    float trebD = (audioTreble * 0.07 + audioTreble * audioTreble * 0.055) * gameNoiseMul;

    float edgeN1 = snoise3(vec3(uv * 0.3, iTime * 0.12)) * bassD * 3.0;
    float edgeN2 = snoise3(vec3(uv * 0.5, iTime * 0.18)) * bassD * 2.0;
    float edgeN3 = snoise3(vec3(uv * 0.9, iTime * 0.3)) * midD * 2.5;
    float edgeN4 = snoise3(vec3(uv * 1.5, iTime * 0.5)) * trebD * 2.0;
    float edgeN5 = snoise3(vec3(uv * 2.5, iTime * 0.8)) * trebD * 1.5;

    float punchD = audioTransient * 0.08;
    float punchNoise = snoise3(vec3(uv * 0.8, iTime * 0.6)) * punchD
                     + snoise3(vec3(uv * 1.4, iTime * 0.9)) * punchD * 0.5;

    float bassSquish = sin(ang * 2.0 + iTime * 0.25) * bassD * 1.8;
    float midSquish = sin(ang * 3.0 - iTime * 0.3) * midD * 1.4
                    + sin(ang * 4.0 + iTime * 0.4) * midD * 0.9;
    float trebSquish = sin(ang * 6.0 + iTime * 0.7) * trebD * 1.2
                     + sin(ang * 9.0 - iTime * 1.0) * trebD * 0.7;

    float breatheVar = breathe * (0.018 + sin(ang * 2.0 + iTime * 0.25) * 0.007);
    float edgeNoise = (edgeN1 + edgeN2 + edgeN3 + edgeN4 + edgeN5 + punchNoise + bassSquish + midSquish + trebSquish + breatheVar) * (1.0 - calm) * arousalScale;

    if (gameMode > 0.01) {
      // Use sin products instead of snoise3 — same low-frequency bubbling, much cheaper on mobile.
      float gameBubble = sin(uv.x * 3.8 + iTime * 0.35) * cos(uv.y * 3.2 - iTime * 0.25) * 0.05
                       + sin(uv.x * 6.1 - iTime * 0.55) * sin(uv.y * 5.7 + iTime * 0.45) * 0.03
                       + sin(ang * 3.0 + iTime * 0.8) * 0.03
                       + sin(ang * 5.0 - iTime * 1.2) * 0.02;
      edgeNoise += gameBubble * gameMode;
    }

    float happyWiggle = (sin(ang * 3.0 + iTime * 1.8) * 0.012 + sin(ang * 5.0 - iTime * 2.5) * 0.008) * happyV;
    float mainSDF = effLen - (dynamicRadius + edgeNoise + happyWiggle + joyPulse * 0.018);

    float vis1 = smoothstep(0.22, 0.58, sin(satSeed        + satSeed2 * 0.71) * 0.5 + 0.5);
    float vis2 = smoothstep(0.22, 0.58, sin(satSeed + 2.399 + satSeed2 * 0.93) * 0.5 + 0.5);
    float vis3 = smoothstep(0.22, 0.58, sin(satSeed + 4.798 + satSeed2 * 1.17) * 0.5 + 0.5);
    float vis4 = smoothstep(0.22, 0.58, sin(satSeed + 7.197 + satSeed2 * 0.59) * 0.5 + 0.5);
    float vis5 = smoothstep(0.22, 0.58, sin(satSeed + 1.847 + satSeed2 * 1.31) * 0.5 + 0.5);
    float vis6 = smoothstep(0.22, 0.58, sin(satSeed + 3.691 + satSeed2 * 0.83) * 0.5 + 0.5);
    float vis7 = smoothstep(0.22, 0.58, sin(satSeed + 5.236 + satSeed2 * 1.07) * 0.5 + 0.5);
    float vis8 = smoothstep(0.22, 0.58, sin(satSeed + 0.618 + satSeed2 * 1.43) * 0.5 + 0.5);

    float orbit1 = dynamicRadius * (0.88 + sin(satSeed2 + 0.3) * 0.04) + audioLevel * 0.4 + audioBass * 0.45 + audioPow * 0.15;
    vec2 bp1 = vec2(cos(iTime * (0.3 + sin(satSeed2 * 0.5) * 0.06) * arousalScale), sin(iTime * (0.3 + sin(satSeed2 * 0.5) * 0.06) * arousalScale)) * orbit1;
    float br1 = (0.07 + audioBass * 0.07 + audioLevel * 0.03 + breathe * 0.01) * vis1;

    float orbit2 = dynamicRadius * (0.85 + sin(satSeed2 + 1.9) * 0.04) + audioLevel * 0.33 + audioMid * 0.35 + audioPow * 0.12;
    vec2 bp2 = vec2(cos(iTime * (-0.22 + sin(satSeed2 * 0.43) * 0.05) * arousalScale + 2.094), sin(iTime * (-0.22 + sin(satSeed2 * 0.43) * 0.05) * arousalScale + 2.094)) * orbit2;
    float br2 = (0.055 + audioMid * 0.06 + audioLevel * 0.025 + breathe * 0.006) * vis2;

    float orbit3 = dynamicRadius * (0.82 + sin(satSeed2 + 3.7) * 0.04) + audioLevel * 0.36 + audioTreble * 0.3 + audioPow * 0.1;
    vec2 bp3 = vec2(cos(iTime * (0.42 + sin(satSeed2 * 0.61) * 0.07) * arousalScale + 4.188), sin(iTime * (0.42 + sin(satSeed2 * 0.61) * 0.07) * arousalScale + 4.188)) * orbit3;
    float br3 = (0.04 + audioTreble * 0.05 + audioLevel * 0.025) * vis3;

    float orbit4 = dynamicRadius * (0.9 + sin(satSeed2 + 5.1) * 0.04) + audioLevel * 0.42 + audioPow * 0.18;
    vec2 bp4 = vec2(cos(iTime * (-0.55 + sin(satSeed2 * 0.37) * 0.08) * arousalScale + 1.047), sin(iTime * (-0.55 + sin(satSeed2 * 0.37) * 0.08) * arousalScale + 1.047)) * orbit4;
    float br4 = (0.025 + audioLevel * 0.06) * vis4;

    float orbit5 = dynamicRadius * (0.84 + sin(satSeed2 + 2.3) * 0.04) + audioLevel * 0.42 + audioBass * 0.35 + audioPow * 0.15;
    vec2 bp5 = vec2(cos(iTime * (0.38 + sin(satSeed2 * 0.53) * 0.06) * arousalScale + 3.5), sin(iTime * (0.38 + sin(satSeed2 * 0.53) * 0.06) * arousalScale + 3.5)) * orbit5;
    float br5 = (0.055 * audioLevel + audioBass * 0.05 + audioPow * 0.03) * vis5;

    float orbit6 = dynamicRadius * (0.92 + sin(satSeed2 + 4.4) * 0.04) + audioLevel * 0.45 + audioTreble * 0.25 + audioPow * 0.18;
    vec2 bp6 = vec2(cos(iTime * (-0.65 + sin(satSeed2 * 0.47) * 0.07) * arousalScale + 5.5), sin(iTime * (-0.65 + sin(satSeed2 * 0.47) * 0.07) * arousalScale + 5.5)) * orbit6;
    float br6 = (0.04 * audioLevel + audioTreble * 0.04 + audioPow * 0.025) * vis6;

    float orbit7 = dynamicRadius * (0.86 + sin(satSeed2 + 6.0) * 0.04) + audioLevel * 0.45 + audioMid * 0.3 + audioPow * 0.2;
    vec2 bp7 = vec2(cos(iTime * (0.25 + sin(satSeed2 * 0.57) * 0.05) * arousalScale + 0.8), sin(iTime * (0.25 + sin(satSeed2 * 0.57) * 0.05) * arousalScale + 0.8)) * orbit7;
    float br7 = (audioPow * 0.06 + audioLevel * 0.03) * vis7;

    float orbit8 = dynamicRadius * (0.94 + sin(satSeed2 + 1.1) * 0.04) + audioLevel * 0.49 + audioBass * 0.35 + audioPow * 0.22;
    vec2 bp8 = vec2(cos(iTime * (-0.4 + sin(satSeed2 * 0.67) * 0.06) * arousalScale + 4.8), sin(iTime * (-0.4 + sin(satSeed2 * 0.67) * 0.06) * arousalScale + 4.8)) * orbit8;
    float br8 = (audioPow * 0.05 + audioBass * 0.035) * vis8;

    float k = 0.11 - audioLevel * 0.03;
    float d = mainSDF;
    d = smin(d, length(uv - bp1) - br1, k * mix(0.4, 1.0, vis1));
    d = smin(d, length(uv - bp2) - br2, k * 0.85 * mix(0.4, 1.0, vis2));
    d = smin(d, length(uv - bp3) - br3, k * 0.7 * mix(0.4, 1.0, vis3));
    d = smin(d, length(uv - bp4) - br4, k * 0.55 * mix(0.4, 1.0, vis4));
    d = smin(d, length(uv - bp5) - br5, k * 0.75 * mix(0.4, 1.0, vis5));
    d = smin(d, length(uv - bp6) - br6, k * 0.6 * mix(0.4, 1.0, vis6));
    d = smin(d, length(uv - bp7) - br7, k * 0.65 * mix(0.4, 1.0, vis7));
    d = smin(d, length(uv - bp8) - br8, k * 0.5 * mix(0.4, 1.0, vis8));

    // Pass 1 — each blob connects to the BASE mass independently.
    // gk is weighted by the blob's own scale so the dome grows smoothly
    // from zero rather than popping in at full size on the first frame.
    // Using d_base (not the running d) prevents rising blobs from distorting
    // the emergence animation of newly forming blobs.
    float d_base = d;
    float gk = k * 5.0;
    if (gb0.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb0), gk * min(gb0.w * 3.0, 1.0)));
    if (gb1.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb1), gk * min(gb1.w * 3.0, 1.0)));
    if (gb2.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb2), gk * min(gb2.w * 3.0, 1.0)));
    if (gb3.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb3), gk * min(gb3.w * 3.0, 1.0)));
    if (gb4.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb4), gk * min(gb4.w * 3.0, 1.0)));
    if (gb5.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb5), gk * min(gb5.w * 3.0, 1.0)));
    if (gb6.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb6), gk * min(gb6.w * 3.0, 1.0)));
    if (gb7.w > 0.005) d = min(d, smin(d_base, gameBlobSDF(uv, gb7), gk * min(gb7.w * 3.0, 1.0)));

    // Pass 2 — blob-to-blob merging with a small radius so blobs that are
    // close to each other merge smoothly, but blobs far apart (e.g. one rising,
    // one just forming) cannot reach each other and cause jank.
    float mk = k * 0.85;
    if (gb0.w > 0.005) d = smin(d, gameBlobSDF(uv, gb0), mk);
    if (gb1.w > 0.005) d = smin(d, gameBlobSDF(uv, gb1), mk);
    if (gb2.w > 0.005) d = smin(d, gameBlobSDF(uv, gb2), mk);
    if (gb3.w > 0.005) d = smin(d, gameBlobSDF(uv, gb3), mk);
    if (gb4.w > 0.005) d = smin(d, gameBlobSDF(uv, gb4), mk);
    if (gb5.w > 0.005) d = smin(d, gameBlobSDF(uv, gb5), mk);
    if (gb6.w > 0.005) d = smin(d, gameBlobSDF(uv, gb6), mk);
    if (gb7.w > 0.005) d = smin(d, gameBlobSDF(uv, gb7), mk);

    float mask = smoothstep(0.01, -0.01, d);

    float normLen = clamp(1.0 + d / (dynamicRadius * 0.85), 0.0, 1.0);
    float zDepth = sqrt(max(1.0 - normLen * normLen, 0.0));

    float nPx = snoise3(vec3(uv * 1.3 + 0.1, iTime * 0.2)) * 0.2 * (1.0 - calm);
    float nPy = snoise3(vec3(uv * 1.3 + 5.0, iTime * 0.2)) * 0.2 * (1.0 - calm);
    vec3 normal = normalize(vec3(uv / max(dynamicRadius, 0.01) + vec2(nPx, nPy) * 0.15, zDepth + 0.1));
    ...
    float spec = pow(max(dot(normal, halfDir), 0.0), specExp) * (0.5 + audioLevel * 1.0 + audioTreble * 0.8);

    float fresnel = pow(1.0 - zDepth, 2.5 - audioTreble * 0.8);

    float noiseSpeed = (0.4 + audioLevel * 0.8) * arousalScale;
    float n0 = snoise3(vec3(uv * (0.6 + audioLevel * 0.3), iTime * noiseSpeed)) * 0.5 + 0.5;
    float interiorN1 = snoise3(vec3(uv * 1.0, iTime * 0.4 * arousalScale)) * 0.5 + 0.5;
    float interiorN2 = snoise3(vec3(uv * 0.5 + 7.0, iTime * 0.2 * arousalScale)) * 0.5 + 0.5;
    ...
    vec3 coreColor = mixOklab(mixOklab(color3 * 1.5, color1 * 0.85, cloudN * 0.55), colBase, mix(interiorN1, interiorN2, 0.4));
    ...
    col += color2 * max(dot(normal, -lightDir), 0.0) * 0.12;
    col += nTreble * colBase;

    col += color2 * light2(0.6 + audioBass * 0.8, 25.0, distance(uv, vec2(cos(iTime * -0.7), sin(iTime * -0.7)) * dynamicRadius * 0.6));
    col += color1 * light2(0.4 + audioTreble * 0.6, 30.0, distance(uv, vec2(cos(iTime * 0.45 + 2.094), sin(iTime * 0.45 + 2.094)) * dynamicRadius * 0.4)) * (0.3 + audioTreble * 0.7);

    // Each satellite blob gets its own rainbow hue in euphoric mode
    float bh1 = max(1.0 - length(uv - bp1) / (br1 * 2.5 + 0.01), 0.0); bh1 *= bh1;
    col += mix(colBase, adjustHue(baseColor1, rainbowSpin + 0.0),   euphoric) * bh1 * 0.3;
    float bh2 = max(1.0 - length(uv - bp2) / (br2 * 2.5 + 0.01), 0.0); bh2 *= bh2;
    col += mix(color2,  adjustHue(baseColor1, rainbowSpin + 45.0),  euphoric) * bh2 * 0.25;
    float bh3 = max(1.0 - length(uv - bp3) / (br3 * 2.5 + 0.01), 0.0); bh3 *= bh3;
    col += mix(color1,  adjustHue(baseColor1, rainbowSpin + 90.0),  euphoric) * bh3 * 0.2;
    float bh5 = max(1.0 - length(uv - bp5) / (br5 * 2.5 + 0.01), 0.0); bh5 *= bh5;
    col += mix(color2,  adjustHue(baseColor1, rainbowSpin + 135.0), euphoric) * bh5 * 0.3;
    float bh6 = max(1.0 - length(uv - bp6) / (br6 * 2.5 + 0.01), 0.0); bh6 *= bh6;
    col += mix(colBase, adjustHue(baseColor1, rainbowSpin + 180.0), euphoric) * bh6 * 0.25;
    float bh7 = max(1.0 - length(uv - bp7) / (br7 * 2.5 + 0.01), 0.0); bh7 *= bh7;
    col += mix(color1,  adjustHue(baseColor1, rainbowSpin + 225.0), euphoric) * bh7 * 0.3;
    float bh8 = max(1.0 - length(uv - bp8) / (br8 * 2.5 + 0.01), 0.0); bh8 *= bh8;
    col += mix(color2,  adjustHue(baseColor1, rainbowSpin + 270.0), euphoric) * bh8 * 0.25;

    float ringFade = smoothstep(dynamicRadius * 0.6, dynamicRadius * 0.93, len);
    col *= mix(1.0, ringFade, calm);

    col = clamp(col * mask, 0.0, 1.0);

    float rim = (1.0 - smoothstep(0.0, 0.06, -d)) * mask * (0.22 + calm * 0.2);
    col += colBase * rim;

    float calmGlow = 0.12 * calm * smoothstep(dynamicRadius + 0.15, dynamicRadius, len) * ringFade;
    col += colBase * calmGlow;

    float glowFade = exp(-max(d, 0.0) * 5.0) * (1.0 - mask);
    col += colBase * glowFade * (0.07 + audioLevel * 0.2);

    float warmGlow = happyV * 0.032 * exp(-max(d, 0.0) * 3.6) * (1.0 - mask);
    col += colBase * warmGlow;

    col += colBase * smoothstep(0.025 + audioBass * 0.03, 0.0, abs(d)) * audioBass * 0.25;

    // Organic drifting fireflies in euphoric mode — all sine-based, no noise, mobile-friendly
    if (euphoric > 0.01) {
      ...
      for (int si = 0; si < 8; si++) {
        ...
        float radBase  = emotionRadius * (1.06 + sin(fi * 1.618) * 0.04);
        float radDrift = sin(iTime * 0.13 + fi * 4.1) * sin(iTime * 0.21 + fi * 2.7) * 0.035;
        vec2  sPos     = vec2(cos(sAng), sin(sAng)) * (radBase + radDrift);
        ...
        vec3 sparkHue = applySaturation(
          adjustHue(baseColor1, rainbowSpin * 0.85 + fi * 43.7),
          1.4
        );
        sparkCol += sparkHue * (sg * 2.2 + sgGlow);
      }
    ...
    if (happyV > 0.01) {
      ...
      float shimmerMask = smoothstep(dynamicRadius * 0.5, dynamicRadius * 0.9, len) * mask;
      col += colBase * shimmer * 0.022 * happyV * shimmerMask;
    }

    col *= 1.0 + joyPulse * 0.09;

    // Warm flush — the orb's internal colours blush toward red as a wave
    // sweeps through, like blood flushing under skin
    if (warmPulse > 0.001) {
      ...
      float dispAxis = axis + nDisp;
      float front = mix(-dynamicRadius * 1.2, dynamicRadius * 1.2, sweepT);
      float bandW = dynamicRadius * 0.8;
      float wave = exp(-((dispAxis - front) * (dispAxis - front)) / (bandW * bandW));
      float strength = wave * warmPulse * mask;
      col *= mix(vec3(1.0), vec3(1.55, 0.52, 0.48), strength * 0.7);
    }
  ...
  vec4 mainImage(vec2 fragCoord) {
    ...
    vec2 tPos = vec2(touchPosX, touchPosY);
    tPos += vec2(randomOffsetX, randomOffsetY);
    tPos = vec2(cr * tPos.x - sr * tPos.y, sr * tPos.x + cr * tPos.y);

    float tDist = length(uv - tPos);
    vec2 tDir = (tDist > 0.001) ? normalize(uv - tPos) : vec2(0.0);
    float fDist = max(tDist - touchSpread, 0.0);

    float approxRadius = mix(0.05, 0.8, emotionSize);
    float surfDist = length(tPos) - approxRadius;
    float onOrb = smoothstep(0.2, -0.05, surfDist);
    float tStr = touchStrength * onOrb;

    float attract = tStr * 0.06 * exp(-fDist * fDist * 16.0);
    uv -= tDir * attract;

    float pressure = max(tStr - 0.2, 0.0) * 0.18 * exp(-fDist * fDist * 10.0);
    uv += tDir * pressure;

    float ripple = tStr * 0.016 * sin(tDist * 18.0 - iTime * 4.5) * exp(-fDist * 4.5);
    uv += tDir * ripple;

    float turb = tStr * 0.022 * exp(-fDist * fDist * 12.0);
    uv.x += turb * snoise3(vec3(uv * 3.0, iTime * 2.0));
    uv.y += turb * snoise3(vec3(uv * 3.0 + 50.0, iTime * 2.0));

    vec2 tVel = vec2(touchVelX, touchVelY);
    tVel = vec2(cr * tVel.x - sr * tVel.y, sr * tVel.x + cr * tVel.y);
    float velMag = length(tVel);
    vec2 velNorm = tVel / (velMag + 0.001);
    float wake = min(velMag * 0.4, 1.0) * tStr * 0.12 * exp(-fDist * 3.5);
    uv += velNorm * wake * (0.3 + 0.7 * max(dot(tDir, velNorm), 0.0));

    float bassDist = audioBass * 0.07;
    uv.x += bassDist * snoise3(vec3(uv * 0.7, iTime * 0.35));
    uv.y += bassDist * snoise3(vec3(uv * 0.7 + 10.0, iTime * 0.4));

    float midDist = audioMid * 0.04;
    uv.x += midDist * snoise3(vec3(uv * 1.5, iTime * 0.8));
    uv.y += midDist * snoise3(vec3(uv * 1.5 + 10.0, iTime * 0.9));

    float trebDist = audioTreble * 0.025;
    uv.x += trebDist * sin(uv.y * 14.0 + iTime * 4.0);
    uv.y += trebDist * cos(uv.x * 14.0 + iTime * 4.0);
  ...
  void main() {
    ...
    float finalAlpha = col.a * emotionOpacity;
